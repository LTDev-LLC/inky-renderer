...
v1.get('/render/:providers?/:raw?', async (c) => {
    let _providers = c.req.param('providers'),
        _mode = {
            w: parseInt(c.req.query('w') ?? c.req.query('width') ?? 1200),
            h: parseInt(c.req.query('h') ?? c.req.query('height') ?? 825),
            mbh: parseInt(c.req.query('mbh') ?? 0),
            fit: c.req.query('f') ?? c.req.query('fit') ?? undefined,
            q: c.req.query('q') ?? c.req.query('quality') ?? undefined,
            transform: c.req.query('transform') !== "false"
        },
    ...
    try {
        ...
        switch (_type) {
            ...
            case "image":
                // Build the image URL
                let [img, params = {}] = await provider.image(data, _mode, c);

                // Assign params over the top of _mode
                _mode = Object.assign(_mode, params);
                ...
                return new Response((await fetch(img, {
                    headers,
                    ...(_mode.transform ? transform(_mode, _headers, _modeFit) : {}),
                })).body, {
                    headers: new Headers([
